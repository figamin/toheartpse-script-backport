...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < 1168; i++) {
    size_t sizeret;
    u_char *data = leafpack_extract(lp, i, &sizeret);

    FILE *fp = fopen(lp->files[i].name, "wb");
    fwrite(data, sizeret, 1, fp);
    fclose(fp);
    free(data);
  }
...
static void extract_table(LeafPack *lp) {
   ...
   */
  u_char *p = lp->addr + lp->size - 24 * lp->file_num + 1701;
